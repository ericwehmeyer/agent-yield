...
**[macOS 2026-08-26] THREE constants in this repo have now been caught
describing a population this repo is not. Check the denominator before quoting
any of them.** (1) **#65** — the "52-call median dispatch" was
`model-migration-kit`'s audit fleet; this repo is median 5, max 30. (2) **#45** —
the daily report's denominator was the *other clone's* commits, over-attributing
2.27× on commits and 6.59× on code lines. (3) **#80** — the cost family's
percentiles were a pooled figure over a project mixture; `COST_DISPATCH` is p46
in one repo on this machine and p100 in another, and the pooled share moved
46% → 18% in one day because the mixture changed. **The shape is always the same:
a number measured on a corpus, quoted without the corpus.** The remedy has also
been the same all three times, and it is never a retune — it is recording the
decomposition next to the aggregate.
...
| | |
|---|---|
| ~~**#23**~~ | **Closed 2026-08-26.** Cost is absolute tokens now — `COST_DISPATCH = 300_000` (p65) / `COST_RESTART = 500_000` (p87) / `COST_STOP = 700_000` (p93), each carrying the share of main-thread calls it fires on. **[Annotated by #80, 2026-08-26: those three percentiles were one pooled figure over a project mixture and describe no project that produced it — 300,000 is p46 in `model-migration-kit` and p100 in `agent-yield`. The constants are unchanged and correct; the percentiles are now ranges. The tokens stand; the parentheses above do not.]** The rest as closed: and a test that fails if a constant loses its percentile. `window` is off the cost path: `cost_band` and `cost_advice` raise `TypeError` if handed one. Three bands because there are three actions; `cost_band` is main-thread only. design.md §5 rewritten. |
| ~~**#80**~~ | **Closed 2026-08-26.** `thresholds.py` recorded each cost constant with one pooled percentile — `COST_DISPATCH` "~35% of main calls (p65), 67% of the bill", macOS, 1,165 calls. **The share is decomposable by project and the pooled number describes none of them.** Every main-thread call under `~/.claude/projects`, 2026-08-26, 2,425 calls over three projects: 300,000 fires on **54% of `model-migration-kit`'s main calls (p46)**, **40% of `Pictures`' (p60)** and **0% of `agent-yield`'s (p100)** — a spread *wider than the macOS-vs-Windows gap the original caveat was written to warn about*. The pooled figure moved **46% → 18%** between calibration day and today with nothing changed about how any session is run: the corpus went from 100% `Pictures`+`model-migration-kit` to 96% `agent-yield`. **The mixture moved, not the behaviour.** Constants **not** retuned, on purpose: #23 put cost in absolute tokens so a threshold would stop being a property of the observer, and a per-repo retune is that error with *project* for *window* — the same 300,000-token call costs the same in either repo, and a repo whose calls never get expensive *should* see the family stay silent. **#68's "peak 295,861, 1.4% short of the constant" is a max-statistic artifact and should not be read as a near-miss:** 8.5% of this repo's calls pass 200K, 1.4% pass 250K, the median *session* peak is 156,455, and the 295,861 is one call in one 127-call session. 300,000 is off the end of this workload, not next to it. Constants now carry a per-project **range** first and the pooled figure second; `test_the_thresholds_are_recorded_with_the_share_of_calls_they_fire_on` is what let the pooled form through (it asked only for a `%` and a `p`) and now requires the decomposition. design.md §5 rewritten. |
| ~~**#81**~~ | **Closed 2026-08-26.** `BASE_RATE_PER_MTOK` carried two entries while `claude-sonnet-5` was **44% of one real day's calls** and `claude-fable-5` another 2.3% — so every dollar figure over a mixed-model window was a lower bound, and two units with different model mixes were comparing the mix as much as the work. **Solved, not looked up:** one `claude -p --output-format json` arm per model, reading a file twice so the run bills a cache *write* and a cache *read* as well as input and output — a rate solved from input alone is one equation that cannot see the multipliers. **`claude-sonnet-5` = $2.00/MTok, `claude-fable-5` = $10.00/MTok**, both 1,000,000-token windows, each reproducing its own arm's `costUSD` to the microdollar ($0.059175 and $0.215854). **The TTL split is load-bearing and was measured, not assumed:** both arms wrote 100% 1h, and priced as 5m writes the same bills solve to $2.6253 and $13.0536 — plausible numbers that are wrong. **The control:** both arms also bill harness-side `claude-haiku-4-5`, and the existing $1.00 reproduces it exactly on both, so the *model* is right and not just the two new constants. Arms archived at `tests/fixtures/arms-81/` the way #33's four were. **#72's dashboard needed re-syncing and its caveat rewriting, not deleting:** the page's `PRICING` hand-copy is pinned to `pricing.py` by a test, but the captured leaf's dollars were computed *before* the rates existed and **cannot be repriced here** — the capture carries raw `unpricedTok` totals only, and the per-model component split lives on the Windows clone. So the figure is still a lower bound, for a different reason, and `unpricedNote` now **reads `PRICING` instead of asserting** — #74's lesson — so the clause flipped from "no rate exists" to "has since been reconciled, but this capture cannot use it" with no edit to the sentence. New test `test_every_rate_in_the_table_is_backed_by_an_arm` turns the module's own doctrine on the table itself: **a fifth rate typed in without an archived run behind it now fails the suite.** 629 tests. |
