...
**And a bug the experiment found by accident: the §12 marker detector has a
false-negative rate near 100% on a textbook brief** — see **#32**. All five Part
E dispatches carried line ranges, an explicit prohibition, a named output path
and a return contract, and `gate` scored them **0 of 3**. This is why "4 of 73
carried all three markers" cannot be read as a briefing rate.

## State of the board

**The tool is built and works.** Nineteen modules, **250 tests**, green on
...
| **#27** | **Dispatch rubric enforcement. Stage 1 shipped**, warning only, `Explore`/`Plan` exempt. **Stage 2 is now blocked twice over, and `--enforce-brief` must stay OFF.** It was already worse-supported after Part C (marker-only enforcement fires on ~95% of dispatches — the RESTART_HARD_FACTOR failure). **#32 is the second block and the worse one: the detector itself is broken.** Stage 2 would have refused all five Part E dispatches — including the two that produced the measurement retracting lever 1. Do not build enforcement on a detector with #32's false-negative rate. |
...
| **#32** | **NEW, 2026-08-25, and it undermines a number already in print. The §12 marker detector fails on a textbook brief.** The five Part E dispatches each carried an explicit line range, a prohibition on exploring, a named output path and a return contract. `missing_markers` returned **all three missing** for every one. Diagnosed per regex, not guessed: (1) **line ranges** — `_LINE_RANGE_RE` *does* match `lines 1-260`; the marker fails because `_NO_EXPLORE_RE` demands the literal word *explore*, and "do not grep or search the repository, do not read any other file" — a strictly stronger prohibition — scores zero. (2) **output path** — fails on a **newline**: `_OUTPUT_PATH_RE` uses `.{0,60}` without `DOTALL`, so `write it to:\n  /path/x.json` misses while the identical words on one line match. Verified both ways. (3) **return contract** — vocabulary again: `at most 3 lines` is not `under \d+ lines`. **All three test for particular wording rather than for the property.** Consequences: **"4 of 73 carried all three markers" is a detector floor, not a briefing rate**, so Part C's marker population is unreliable in the direction of under-counting, and #27 Stage 2 is unbuildable until this is fixed. **Fix the regexes, then re-run `agents` and re-score Part C before quoting either number again.** |
