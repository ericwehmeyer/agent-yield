...
`subagent_type` `general-purpose`. Dispatch EXACTLY ONE agent and give it all 23
slices. That agent does all three steps for all 23 and returns the JSON fragment
for all 23 and nothing else; you assemble the final object from what comes back.
The packing is fixed by this paragraph and is not your call.
